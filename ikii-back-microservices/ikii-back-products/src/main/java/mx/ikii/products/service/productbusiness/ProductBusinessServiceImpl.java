...
public class ProductBusinessServiceImpl implements IProductBusinessService {
  ...
  @Override
  public List<ProductBusiness> findMostSelledProductsByBussinessId(Pageable pageable,
      ObjectId bussinessId) {
    return productRepository.findAllByBusinessId(pageable, bussinessId);
  }

  @Override
  public List<ProductBusiness> filterProduct(Pageable pageable, ProductFilter productFilter) {
    return productRepositoryCustom.searchByKeywords(productFilter);
  }

  @Override
  public ProductBusiness findByBusinessId(ObjectId businessId) {
    return productRepository.findByBusinessId(businessId).orElseThrow(
        () -> new ResourceNotFoundException(businessId.toHexString(), ProductBusiness.class));
  }
